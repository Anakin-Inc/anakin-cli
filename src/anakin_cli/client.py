...
class AnakinClient:
    ...
    def start_url_scrape(
        ...
        *,
        ...
        resp = self.session.post(f"{BASE_URL}/url-scraper", json=body)
        return self._handle_response(resp)
    ...
    def get_url_scrape_result(self, job_id: str) -> dict:
        ...
        resp = self.session.get(f"{BASE_URL}/url-scraper/{job_id}")
        return self._handle_response(resp)

    def get_agentic_search_result(self, job_id: str) -> dict:
        """GET /v1/agentic-search/{id} — poll research job status."""
        resp = self.session.get(f"{BASE_URL}/agentic-search/{job_id}")
        return self._handle_response(resp)
